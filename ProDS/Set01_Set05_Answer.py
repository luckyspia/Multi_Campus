...
#%%
# 시험에서 자주 사용되는 패키지
# - pandas, numpy, scipy.stats, statsmodels, sklearn(scikit-learn)
...
q2=data2.copy()
# np.where(조건)
# np.where(조건[True/False], 참인경우 실행[함수(식), 상수, 시리즈], 거짓인 경우 실행)
...
# 평가: 유의수준 체크
# 유의수준 0.05 하에서 p-value가 유의수준 보다 작은지 확인, 유의수준보다 작으면 귀무가설 기각
# 즉, 상관이 있다는 의미 
#%%
...
# 3. 육류 종류별로 연도를 기준으로 쌍이되도록 데이터를 설계
# 4. 육류 종류별로 대응 t 검정 수행(pvale 수집->육류 종류 도출)
from scipy.stats import ttest_rel
