...
q2_corr=data6[var_list].corr().drop('price')['price'].abs()

q2_corr.idxmax(), q2_corr.idxmin()
...
# 모형에 남는 변수는 모두 몇 개인가?
(lm1.pvalues.drop('Intercept') < 0.05).sum() # 13

# 이 때 음의 회귀계수를 가지는 변수는 몇 개
(lm1.params[lm1.pvalues < 0.05].drop('Intercept') < 0).sum()

# 답: 13, 2


# [참고] 값 확인
temp=lm1.params[lm1.pvalues < 0.05].drop('Intercept')
temp[temp < 0]
#%%
...
round(q1.drop('Chance_of_Admit')['Chance_of_Admit'].abs().max(),3)
...
# 2. 그룹별로 CGPA 점수를 묶음 -> 두 집단 간의 평균 차이 검정 
# - 등분산으로 가정: 독립인 2표본 t 검정
...
# 3. 검정통계량의 추정치를 소수점 셋째 자리에서 반올림하여 소수점 둘째 자리까지
# 기술
...
var_list= q3.columns.drop(['Serial_No', 'Chance_of_Admit', 'target'])

logit=LogisticRegression(solver='liblinear', random_state=123)
logit.fit(q3[var_list], q3['target'])

# 3. 로지스틱 회귀계수의 절대값이 가장 큰 변수와 그 값을 기술
...
q1=data8.State.value_counts(normalize=True)
q1.sort_index(ascending=True).values
...
# 3. Age_gr, Gender, Customer_Type, Class 변수가 satisfaction=> 독립성 검정
...
q1_tab['day']=q1_tab['km'] / q1_tab['age_in_days']

# 3. 가장 낮은 값을 가진 모델이 가장 큰 값을 가진 모델에 대한
# 비율
round(q1_tab['day'].min() / q1_tab['day'].max(), 2)
...
gr_max=q2[q2.model == q1_tab['day'].idxmax()]['day'].dropna()
gr_min=q2[q2.model == q1_tab['day'].idxmin()]['day'].dropna()

# 2. ttest 수행(등분산을 가정하고)
from scipy.stats import ttest_ind

ttest_out=ttest_ind(gr_max, gr_min, equal_var=True)

# 3. p-value를 소수점 세자리 이하는 버리고 소수점
...
# 3. 반복문을 이용해서 회귀모델 생성 : 모델들이 다 생성되도록 유도 
from sklearn.linear_model import LinearRegression
