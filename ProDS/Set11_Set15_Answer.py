...
anova1_out=f_oneway(q1[2015], q1[2016], q1[2017])
anova1_out.statistic

# 답: 0.0042

q3=q1.drop(columns='ratio').melt()
anova2=ols('value~C(year)', q3).fit()

from statsmodels.stats.anova import anova_lm

anova_lm(anova2)['F'][0] # 0.0042
...
# 5. t 검정 수행:등분산 가정을 만족하지 않는다
from scipy.stats import ttest_ind

ttest_out=ttest_ind(q2[1].dropna(), q2[0].dropna(), equal_var=False)
...
q3_2.columns=['Date', 'day_name','value']

from statsmodels.formula.api import ols
from statsmodels.stats.anova import anova_lm


anova1=ols('value~day_name', q3_2).fit()

round(anova_lm(anova1)['PR(>F)'][0], 2)

# 답: 0.52

from statsmodels.stats.multicomp import pairwise_tukeyhsd

print(pairwise_tukeyhsd(q3_2['value'], q3_2['day_name']))
